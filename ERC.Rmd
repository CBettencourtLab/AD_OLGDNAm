...
design_ERC <- model.matrix(~0+Sample_group+slide+DoubleN+Sox10P+age, data=ERC_samples0)
dim(design_ERC)
...
fit = lmFit(M_matrix_ERC_age, designSv)

contMatrix <- makeContrasts(Sample_groupAlzheimer-Sample_groupControl, levels = designSv)



#contMatrix_ERC <- makeContrasts("Sample_groupAlzheimer-Sample_groupControl", levels = designSv)

#Checking the fit with SVD 

mAdj.fit_ERC    <- fit$coefficients[,-c(1:2)]

mAdj_ERC        <- as.matrix(M_matrix_ERC_age) - mAdj.fit_ERC %*% t(designSv,-c(1:2)])
